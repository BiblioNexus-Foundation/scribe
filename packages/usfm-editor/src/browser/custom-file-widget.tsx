...
export class CustomFileWidget extends ReactWidget implements Saveable {
  ...
  protected init(): void {
    this.id = CustomFileWidget.ID;
    this.title.closable = true;
    this.addClass('custom-file-widget');
    this.update();
  }
  ...
  public activate(): void {
    super.activate();
    this.node.focus();
    this.title.className = 'custom-tab-active';
    this.update();
  }
  ...
  protected onAfterHide(msg: Message): void {
    ...
    this.title.className = 'custom-tab';
  }
  ...
  protected render(): React.ReactNode {
    return (
      <div className="custom-file-widget" style={{ cursor: 'text' }}>
        {this.processedContent && (
          <LexicalEditor
            usjInput={this.processedContent}
            isDirty={this.dirty}
  ...
  dispose(): void {
    if (this.onDirtyChangedEmitter) {
      this.onDirtyChangedEmitter.dispose();
    }
    if (this.onContentChangedEmitter) {
