...
const argv = yargs(hideBin(process.argv))
  .option("executable", {
    alias: "e",
    type: "string",
    default: "TheiaIDE.AppImage",
    description: "The executable for which the checksum needs to be updated",
  })
  .option("yaml", {
    alias: "y",
    type: "string",
    default: "latest-linux.yml",
    description: "The yaml file where the checksum needs to be updated",
  })
  .option("platform", {
    alias: "p",
    type: "string",
...
async function execute(): Promise<void> {
  ...
  const executablePath = path.resolve(__dirname, "../dist/", executable);

  const yamlPath = path.resolve(__dirname, "../dist/", yaml);
...
function updatedPath(toUpdate: string, version: string, platform: string): string {
  ...
  return (
    ...
    platform +
    "/" +
    toUpdate.substring(0, extensionIndex) +
    "-" +
    version +
    toUpdate.substring(extensionIndex)
  );
}
